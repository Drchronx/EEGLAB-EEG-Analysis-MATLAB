...
%  功能: 支持【组内 (Paired)】和【组间 (Independent)】切换
...
def = {'[4 7]', '[200 600]', '[-1000 4 1996]', '[1 1 30]', 'Cond1', 'Cond2'};
...
if is_paired
    ...
    stat_label = 'Paired t-test';
else
    file_titles = {'Group 1', 'Group 2'};
    stat_label = 'Independent t-test';
end
...
t_values = zeros(1, nPairs); % 存一下 t 值，用于画图颜色（红/蓝区分方向）
...
if strcmp(choice_exp, 'Yes')
    ...
    if n_sig > 0
        ...
        for s = 1:length(sig_idx)
            ...
            % 技巧：存入 t 值而不是 1
            % 这样在 BrainNet Viewer 里，可以设置 Colormap
            % t > 0 (Condition 1 > 2) 显示红色
            % t < 0 (Condition 1 < 2) 显示蓝色
            val = t_values(idx); 
        ...
        msgbox({'导出完成！'; ''; ...
                '提示：Edge 文件中保存的是 t 值。'; ...
                '在 BrainNet Viewer 中设置 Edge Color 为 Jet 或 CoolHot，'; ...
                '即可用颜色区分增强/减弱的连接。'});
    else
